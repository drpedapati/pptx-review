...
#   make test         # Run test against sample presentation
#   make clean        # Remove build artifacts

BINARY_NAME  := pptx-review
VERSION      := 1.2.0
BUILD_DIR    := build
INSTALL_DIR  := /usr/local/bin
GH_REPO      := drpedapati/pptx-review
...
.PHONY: build install all docker test clean help release release-assets update-taps
...
test: build
	...
	@if [ ! -f "$(TEST_PPTX)" ]; then \
		echo "Usage: make test TEST_PPTX=/path/to/presentation.pptx"; \
		exit 1; \
	fi
	$(BUILD_DIR)/$(BINARY_NAME) "$(TEST_PPTX)" examples/sample-edits.json -o $(BUILD_DIR)/test_output.pptx
	@echo ""
	@ls -lh $(BUILD_DIR)/test_output.pptx

## test-read: Read a presentation (JSON output)
test-read: build
	@if [ -f "$(TEST_PPTX)" ]; then \
		$(BUILD_DIR)/$(BINARY_NAME) "$(TEST_PPTX)" --read --json; \
	else \
		echo "Usage: make test-read TEST_PPTX=/path/to/presentation.pptx"; \
	fi

## test-dry: Dry-run test (no modifications)
test-dry: build
	@if [ -f "$(TEST_PPTX)" ]; then \
		$(BUILD_DIR)/$(BINARY_NAME) "$(TEST_PPTX)" examples/sample-edits.json --dry-run; \
	else \
		echo "Usage: make test-dry TEST_PPTX=/path/to/presentation.pptx"; \
	fi
...
update-taps: release-assets
	...
	for tap in $(HOMEBREW_TAPS); do \
		...
		sed -i '' "s|pptx-review [0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*|pptx-review $(VERSION)|" $$FORMULA; \
...
## help: Show this help
help:
	@echo "pptx-review $(VERSION) — PowerPoint editing tool"
	@echo ""
	@echo "Targets:"
	...
	@echo "  make test TEST_PPTX=presentation.pptx   # Run test"
	@echo "  make test-read TEST_PPTX=slides.pptx    # Read presentation"
